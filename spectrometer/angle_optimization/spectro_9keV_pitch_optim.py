...
xtal2_pitch -= radians(pitch_corr) # manually optimised if energy is off
...
#                np.savetxt(fout+'.dat', np.array([x,y]).T, header=stats)
    ...
    print('\n\nIntensity: ')
    print(intensity)
    optim_res = f'{dat_dir}optim.dat'
    with open(optim_res, "a") as f:
        data = '{},{}\n'.format(str(pitch_corr), str(intensity))
        f.write(data)
    print('\n\n\n')
