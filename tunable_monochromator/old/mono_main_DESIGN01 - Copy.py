...
E0 = 4400 # [eV]
dE = 0.1 # [eV]
step = 0.005 # used for distE='lines' only
subfolder = ''
save_prefix = 'FULL_f300_'
save_suffix = ''
build_oe = [True, True, False, False, True, True] # decide whether an element is built (in order)
do_plot = 'all'
...
d = [ds0, d01, d1m0, dm0m1, dm12, d01] # distance to previous element (absolute)
beam_angle = [Si_tth, -Si_tth, 2*m_pitch, -2*m_pitch, -Si_tth, Si_tth] # angle that the beam make at each crystal
angle = np.cumsum(beam_angle) # beam angle in (y-z) global frame afer each element (0 = parallel to y)
position_roll = [0, np.pi, 0, np.pi, np.pi, 0] # element facing up or down

miscuts = [0, miscut, None, None, 0, -miscut] # None for non-crystal elements
qp = [None, None, 'q', 'q', None, None] # None for non-curved mirror elements
