...
subfolder = 'DESIGN01_V2/'
save_prefix = '1.0_'
save_suffix = ''
build_oe = [True, True, True, True, True, True] # decide whether an element is built (in order)
do_plot = 'all'
...
### Optical elements ###
oes = ['crystal', 'crystal', 'parabolic_mirror',
...
qp = [None, None, 'q', 'p', None, None] # None for non-curved mirror elements

beam_angle = np.array(
    [Si_tth, -Si_tth, 2*m_pitch, -2*m_pitch, -Si_tth, Si_tth]) # angle that the beam make at each crystal
ttheta_corr = []
for misc in miscuts:
    if misc is not None:
