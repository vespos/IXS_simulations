...
save_prefix = 'test_'
save_suffix = ''
build_oe = [True, True] # decide whether an element is built (in order)
do_plot = 'all'
no_div = False
...
### Optical elements ###
oes = ['parabolic_mirror', 'parabolic_mirror'] # oes category
...
d = [ds0, d01] # distance to previous element (absolute)
position_roll = [0, np.pi] # element facing up or down
miscuts = [None, None] # None for non-crystal elements
q = [f,f] # None for non-curved mirror elements
p = [ds,ds] # None for non-elliptical mirror elements
pitch = [m_pitch, -m_pitch]
beam_angle = np.array([2*m_pitch, -2*m_pitch])
...
focus_kwargs = {
    'name': 'on_focus',
    'center': s_kwargs[0]['center']+n[1]*f,
...
class LCLS_source(GeometricSource):
    ...
    def LCLS_source_properties(self):
        ...
        if E0>=2000 and E0<8000:
            FWHM = 45*(5000/E0) #um
            div = 3.4*(5000/E0) #urad
        if E0>=8000 and E0<=13000:
            FWHM = 37*(8000/E0)**0.25 #um
...
def run_process(bl):
    ...
    beam_on_focus = bl.on_focus.expose(beam=beam[1])
    outDict['on_focus'] = beam_on_focus
